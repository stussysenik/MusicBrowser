...
/// Lightweight song model for demo/testing without MusicKit dependency.
struct DemoSong: Identifiable, FilterableByLetter {
    let id: String
    let title: String
    ...
    init(_ title: String, artist: String, album: String = "", duration: TimeInterval = 210, genres: [String] = ["Pop"], plays: Int = 0, year: Int = 2024) {
        self.id = "\(title)-\(artist)".lowercased().replacingOccurrences(of: " ", with: "-")
        self.title = title
        self.artistName = artist
        self.albumTitle = album.isEmpty ? "\(title) - Single" : album
...
/// Pre-built test dataset covering every letter A-Z plus "#" for numeric titles.
